// Macau 2D Engine - Final Fixed Version
function runEngine() {
    const now = new Date();
    ...
    let displayData = generateNumbers(isFrozen ? Math.floor(((freezeTime - 1) * 3600 + 59 * 60 + 54) / 6) : cycleIndex);
...
function generateNumbers(index) {
    ...
    const sNum = (random(index) * (1999.99 - 1000.00) + 1000.00).toFixed(2);
    const vNum = (random(index + 1) * (59999.99 - 10000.00) + 10000.00).toFixed(2);
    return { res: sNum.slice(-1) + vNum.split('.')[0].slice(-1), set: sNum, val: vNum };
}

function updateUI(data, h, m, s) {
    // ၁။ နာရီကို ဖြည့်စွက်ခြင်း
    const clock = document.getElementById('live-clock');
    if (clock) clock.innerText = ${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')};

    // ၂။ ဂဏန်းအကြီးကို ဖြည့်စွက်ခြင်း
    const main = document.getElementById('main-num');
    if (main) main.innerText = data.res;

    // ၃။ ကျန်တဲ့ အကွက်အသေးလေးများကို ဖြည့်စွက်ခြင်း
